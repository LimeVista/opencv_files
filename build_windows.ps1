function BuildForWindows($platform, $vcpkgPath, $runMsbuild) {
    ...
    cmake -G "Visual Studio 17 2022" `
        ...
        -D OPENCV_FFMPEG_USE_FIND_PACKAGE=FFMPEG `
        -D OPENCV_FFMPEG_SKIP_DOWNLOAD=ON `
...
If ((Resolve-Path -Path $MyInvocation.InvocationName).ProviderPath -eq $MyInvocation.MyCommand.Path) {
    ...
    Invoke-Expression "${vcpkgPath}\vcpkg.exe install ffmpeg[x264,x265]:${platform}-windows-static" -ErrorAction Stop
